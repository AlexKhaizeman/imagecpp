...
void channelCompare(string path, string info, unsigned short **channel_R, unsigned short **channel_R_S, int width, int height){
	dump(info);
	...
	for (std::list<unsigned short>::iterator it = colours_.begin(); it != colours_.end(); ++it){
    	dump(to_string(*it) + " is colour from 1");
    }

	for (std::list<double>::iterator it = part_.begin(); it != part_.end(); ++it){
    	dump(to_string(*it) + " is colour part of 1");
    }

    dump("second picture");

	for (std::list<unsigned short>::iterator it = colours_S.begin(); it != colours_S.end(); ++it){
    	dump(to_string(*it) + " is colour from 2");
    }

	for (std::list<double>::iterator it = part_S.begin(); it != part_S.end(); ++it){
    	dump(to_string(*it) + " is colour part of 2");
    }
}
...
int main(int argc,char **argv){
	...
	try{
		...
		// channelCompare(string path, string info, unsigned short **channel_R, unsigned short **channel_R_S, int width, int height){
		channelCompare(path, "comparing test", channel_G_S, channel_R_S, width, height);
